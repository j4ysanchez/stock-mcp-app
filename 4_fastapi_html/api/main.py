...
Connects to the MCP server via HTTP/SSE, exposes a POST /chat SSE endpoint,
and serves the vanilla HTML frontend via StaticFiles.

Architecture:
  Browser  -->  POST /chat (SSE stream)  -->  Claude  -->  MCP server (port 8001)
...
# ---------------------------------------------------------------------------
# Routes — must be defined BEFORE the StaticFiles mount
# ---------------------------------------------------------------------------
...
# StaticFiles mount last so /chat takes priority.
